...
每道题每次进入都会生成独立作答记录，保存答案修改、提交时间、对错、过程备注、语音转写、卡点标签和草稿图片。题目旁的“我的过程”默认折叠；历史可在“我的 → 练习记录 → 作答记录”查看。图片 Blob 保存在浏览器 IndexedDB，其他过程字段保存在 localStorage，语音仅保存识别后的文字。选中页面文字时会出现“添加到闪卡”浮动按钮；“我的闪卡”可编辑和删除卡片。每日任务支持开始、暂停、继续、完成，并保留每次计时区间。

如果需要让 ChatGPT 的自定义 MCP 插件派发任务、管理闪卡或读取作答记录，在“我的 → 每日任务”生成设备专属 MCP 地址，并将完整地址填入 ChatGPT 的 MCP 连接设置。打开每日任务页会同步 AI 派发的任务；打开闪卡页会双向同步闪卡。做题记录同步默认关闭，需在每日任务页主动开启，开启后会同步过程文字与草稿图片。MCP 工具包括 `assign_daily_tasks`、闪卡增删改查、`list_attempts`、`get_attempt` 和按需读取图片的 `get_attempt_image`。MCP 地址是一个可撤销的设备访问凭证，持有地址的人可以访问该设备已同步的数据；请勿公开。点击“撤销 MCP 地址”可删除此设备的云端任务、闪卡、作答记录和草稿图片，本地数据保留。

MCP 服务部署在 Cloudflare Worker：`scripts/deploy-cf.sh` 会应用 D1 迁移并确保 R2 草稿图片桶存在。自定义部署需配置 `VITE_DAILY_TASKS_RELAY` 指向该 Worker 的 HTTPS 域名（默认使用项目的 Cloudflare 域名）。该服务不调用 AI API；ChatGPT 通过 Streamable HTTP 连接 `/mcp/<设备凭证>`。本地/自托管网页在未连接该 Worker 时，做题记录、闪卡和计时仍可离线使用。
...
过了喵支持 **Windows、macOS、Linux、Android 和 Web**（iOS 需自行打包）。**一句 Docker 命令**即可部署网页版。题库、错题本和练习统计默认存储在本地；在线考试和用户主动启用的 MCP 同步会使用服务端存储。桌面端 API 密钥使用 **AES-256-GCM** 加密存储。界面自动跟随系统语言（中文/英文），一键切换。
